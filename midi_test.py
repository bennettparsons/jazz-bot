...
MyMIDI = MIDIFile(1, adjust_origin=False)  # One track, defaults to format 1 (tempo track is created
                      # automatically)
...
M = music_theory('C')
chords = M.build_progression(['I', 'V', 'I'])

# build major scale of quarter notes
...
for chord in chords:
	for pitch in chord:
		MyMIDI.addNote(track, channel, pitch, time + i, 4, volume)
	i += 4
